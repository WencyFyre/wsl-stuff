...
int bus_main(int pipe_in, int pipe_out, int bus_id)
{
    ...
    if (bus_id)
    {
        sigqueue(getppid(), SIGUSR1, s_value_int);
    }
    else
    {
    ...
    printf("\n");
    printf("%li írva, %i\n",write(pipe_out, buffer, sizeof(buffer)), bus_id);
    //printf("child: kírtam az adatokat: %i\n", bus_id);
    //fflush(stdout);
    return 2;
}
...
int main_vaccinated(Storage *storage)
{
    ...
    storage_getPeopleVacc(storage, 0, ids[0]);
    storage_getPeopleVacc(storage, 5, ids[1]);
    for (int i = 0; i < 2; i++)
    {
    ...
    for (int i = 0; i < 2; i++)
        if (buses[i].exist)
        {
            ...
            waitpid(buses[i].pid, &status, 0);
            while (!WIFEXITED(status))
            {
                printf("waiting for pid: %i\n", buses[i].pid);
                pid_t status = waitpid(buses[i].pid, NULL, 0);
            }
            printf("child with%i  pid ended\n", buses[i].pid);
        };
    fflush(stdout);
    Line line;
    line.vaccinated = 1;
